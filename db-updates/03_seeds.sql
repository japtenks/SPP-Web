...
SET @help_troubleshoot_message := '[b]How to troubleshoot without wasting time[/b]<br />\r\nThe fastest way to troubleshoot this site is to work from the outside in. Start with the selected realm and basic runtime assumptions, then move to permissions and optional services, then move to cache, and only after that start assuming you have a code or schema bug. That order matches the kinds of issues this environment produces most often.<br />\r\n<br />\r\n[b]First checks[/b]<br />\r\nIf the site loads but data looks wrong, check expansion, default realm ID, and DB names first. If admin actions do not work, check SOAP and account permissions. If account-linked pages look incomplete, run the identity backfill. If forum or admin tables are missing, verify the realmd patch was applied to the correct website-owned realmd database.<br />\r\n<br />\r\n[b]Check the error log file[/b]<br />\r\nBefore guessing, open `storage/logs/error_log.txt` and read the newest entries. That file is the first place to look when a page goes blank, a form action fails, or a feature behaves differently than expected. It is often faster to read the latest error there than to keep retrying the same page without new information.<br />\r\n<br />\r\n[b]Cache and Windows-specific issues[/b]<br />\r\nIf pages still show old content after updates or reseeds, clear website cache files and reload. On Windows installs, confirm Apache is pointing at the right `DocumentRoot`, add the default SPP PHP and MySQL binaries to your `PATH` if you rely on CLI tools, and stop the launcher or web service before replacing website files so you do not end up with a half-updated copy on disk.<br />\r\n<br />\r\n[b]When to escalate further[/b]<br />\r\nIf you have already checked runtime mapping, cache, patches, backfill, permissions, and the latest entries in `storage/logs/error_log.txt`, then it is reasonable to start looking at schema drift or code-level regressions. Until then, the simpler explanation is usually the correct one on an SPP-style local stack.';
