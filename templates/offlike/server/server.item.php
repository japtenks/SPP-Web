...
if (!function_exists('spp_item_set_bonus_description')) {
    function spp_item_set_bonus_description(int $realmId, array $spell): string
    {
        ...
        $durationSeconds = spp_item_set_spell_duration_seconds($realmId, (int)($spell['ref_spellduration'] ?? 0));
        $durationText = $formatDuration($durationSeconds);
        $radiusText = spp_item_set_spell_radius_text($realmId, (int)($spell['effect_radius_index_1'] ?? 0));
...
if (!function_exists('get_itemset_data')) {
    ...
    {
        try {
            $realmId = max(1, (int)($GLOBALS['realmId'] ?? ($_GET['realm'] ?? 1)));
            $armoryPdo = spp_get_pdo('armory', $realmId);
            $worldPdo = spp_get_pdo('world', $realmId);
        } catch (Throwable $e) {
            error_log('[item set detail] PDO bootstrap failed: ' . $e->getMessage());
        ...
        $row = $setStmt->fetch(PDO::FETCH_ASSOC) ?: null;
        if (!$row) {
            return ['id' => $setId, 'name' => 'Unknown Set', 'items' => [], 'bonuses' => []];
        }
        ...
        if ($itemIds) {
            ...
            $itemStmt = $worldPdo->prepare("SELECT `entry`,`name`,`InventoryType`,`displayid`,`Quality` FROM `item_template` WHERE `entry` IN ({$placeholders})");
            $itemStmt->execute($itemIds);
            foreach (($itemStmt->fetchAll(PDO::FETCH_ASSOC) ?: []) as $entry) {
        ...
        $iconByDisplayId = [];
        if ($displayIds) {
            $displayIdsList = array_keys($displayIds);
            $placeholders = implode(',', array_fill(0, count($displayIdsList), '?'));
            $iconStmt = $armoryPdo->prepare("SELECT `id`,`name` FROM `dbc_itemdisplayinfo` WHERE `id` IN ({$placeholders})");
            $iconStmt->execute($displayIdsList);
        ...
        if ($bonusSpellIds) {
            ...
            $spellStmt = $armoryPdo->prepare("SELECT * FROM `dbc_spell` WHERE `id` IN ({$placeholders})");
            $spellStmt->execute($bonusSpellIds);
        ...
        if ($spellIconIds) {
            ...
            $spellIconStmt = $armoryPdo->prepare("SELECT `id`,`name` FROM `dbc_spellicon` WHERE `id` IN ({$placeholders})");
            $spellIconStmt->execute($spellIconIdList);
        ...
        foreach ($bonusMeta as $meta) {
            ...
            $bonuses[] = [
                ...
                'desc'   => spp_item_set_bonus_description($realmId, $spell),
...
<script>
var modernItemTooltipNode = null;
var modernItemTooltipCache = new Map();
var modernItemTooltipRequestToken = 0;

function modernItemTooltipEnsure() {
  if (modernItemTooltipNode) {
    return modernItemTooltipNode;
  }
  var tip = document.createElement('div');
  tip.id = 'modern-item-tooltip';
  tip.className = 'modern-item-tooltip-host talent-tt';
  tip.style.display = 'none';
  document.body.appendChild(tip);
  modernItemTooltipNode = tip;
  return tip;
}

function modernItemTooltipLoadingHtml() {
  return '<div class="modern-item-tooltip modern-item-tooltip-loading">Loading item tooltip...</div>';
}

function modernItemTooltipShow(event, html) {
  var tip = modernItemTooltipEnsure();
  tip.innerHTML = html;
  tip.style.display = 'block';
  modernItemTooltipMove(event);
}

function modernRequestTooltip(event, itemId, realmId) {
  var cacheKey = String(realmId) + ':' + String(itemId);
  if (modernItemTooltipCache.has(cacheKey)) {
    modernItemTooltipShow(event, modernItemTooltipCache.get(cacheKey));
    return;
  }

  var requestToken = ++modernItemTooltipRequestToken;
  modernItemTooltipShow(event, modernItemTooltipLoadingHtml());
  ...
  fetch('index.php?n=server&sub=itemtooltip&nobody=1&item=' + encodeURIComponent(itemId) + '&realm=' + encodeURIComponent(realmId), {
    ...
    .then(function (html) {
      if (requestToken !== modernItemTooltipRequestToken) {
        return;
      }
      var resolved = html && html.trim() !== '' ? html : modernItemTooltipLoadingHtml();
      modernItemTooltipCache.set(cacheKey, resolved);
      var tip = modernItemTooltipEnsure();
      if (tip.style.display !== 'none') {
        tip.innerHTML = resolved;
      }
    })
    .catch(function () {
      if (requestToken !== modernItemTooltipRequestToken) {
      ...
      var fallback = '<div class="modern-item-tooltip modern-item-tooltip-loading">Unable to load tooltip.</div>';
      modernItemTooltipCache.set(cacheKey, fallback);
      var tip = modernItemTooltipEnsure();
      if (tip.style.display !== 'none') {
        tip.innerHTML = fallback;
      }
    });
}

function modernItemTooltipMove(event) {
  var tip = modernItemTooltipEnsure();
  if (tip.style.display === 'none') {
    return;
  }
  ...
  var maxLeft = Math.max(viewportPadding, window.innerWidth - tipWidth - viewportPadding);
  var maxTop = Math.max(viewportPadding, window.innerHeight - tipHeight - viewportPadding);
  var left = event.clientX + offsetX;
  var top = event.clientY + offsetY;

  if (left + tipWidth > window.innerWidth - viewportPadding) {
    left = event.clientX - tipWidth - offsetX;
  }
  if (left < viewportPadding) {
    left = Math.round((window.innerWidth - tipWidth) / 2);
  }
  if (left < viewportPadding) {
  ...
  if (top + tipHeight > window.innerHeight - viewportPadding) {
    top = event.clientY - tipHeight - offsetY;
  }
  if (top < viewportPadding) {
    top = Math.round((window.innerHeight - tipHeight) / 2);
  }
  if (top < viewportPadding) {
    top = viewportPadding;
  }
...
function modernHideTooltip() {
  modernItemTooltipRequestToken += 1;
  if (modernItemTooltipNode) {
    modernItemTooltipNode.style.display = 'none';
  }
}
        ...
        <?php if ($setItems): ?>
          <div class="item-set-piece-list item-set-piece-list--with-icons">
            <?php foreach ($setItems as $setPiece): ?>
              <a class="item-set-piece item-set-piece--full" href="<?php echo htmlspecialchars(spp_item_detail_url((int)$realmId, (int)($setPiece['entry'] ?? 0), array('type' => 'items', 'search' => (string)($_GET['search'] ?? ''), 'quality' => (string)($_GET['quality'] ?? ''), 'item_class' => (string)($_GET['item_class'] ?? ''), 'slot' => (string)($_GET['slot'] ?? ''), 'min_level' => (string)($_GET['min_level'] ?? ''), 'max_level' => (string)($_GET['max_level'] ?? ''), 'p' => (string)($_GET['p'] ?? 1), 'per_page' => (string)($_GET['per_page'] ?? 25), 'sort' => (string)($_GET['sort'] ?? 'featured'), 'dir' => (string)($_GET['dir'] ?? 'DESC')))); ?>" onmousemove="modernItemTooltipMove(event)" onmouseover="modernRequestTooltip(event, <?php echo (int)($setPiece['entry'] ?? 0); ?>, <?php echo (int)$realmId; ?>)" onmouseout="modernHideTooltip()">
          ...
          <div class="item-set-bonus-list">
            <?php foreach ($setBonuses as $bonus): ?>
              <div class="item-set-bonus">
                ...
                <span><?php echo htmlspecialchars((string)($bonus['desc'] ?? ($bonus['description'] ?? ''))); ?></span>
              </div>
            <?php endforeach; ?>
          </div>
        <?php else: ?>
