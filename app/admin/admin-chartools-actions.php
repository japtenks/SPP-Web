        ...
        if (!spp_admin_chartools_validate_csrf()) {
            ...
            if (empty($selectedPack)) {
                ...
                try {
                    $mangos = new Mangos;
                    ob_start();
                    if (($selectedPack['kind'] ?? 'database') !== 'database') {
                    ...
                    error_log('[admin.chartools] Item pack delivery failed: ' . $deliveryError);
                }
                ...
                if ($sendOk) {
                    ...
                    $deliveryMessageHtml = '<div class="admin-tool-msg error">The item pack could not be mailed. This realm schema is missing one or more legacy mail/item columns required by the current delivery helper.</div>';
                }
            }
        }
    }
