...
/**
 * Returns an English resource file.
 *
 * This keeps legacy resource lookup paths working while the site remains
 * English-first.
 *
...
function lang_resource($name){
    ...
    $sourcefiles = array(
        "lang/$resourcename/en.$resourceext",
        "lang/$resourcename/{$resourcename}_en.$resourceext",
        "lang/en.$resourcename.$resourceext",
    );
...
function check_image($img_file){
    $maximgsize = explode('x',(string)spp_config_generic('imageautoresize', '500x500'));
...
function log_error($string, $choise=1){
  ...
  if($choise == 1){
    $fh = fopen(spp_storage_path('logs/core_error_log.log'), 'a');
...
function get_realm_byid($id){
    ...
    if((int)spp_config_generic('use_local_ip_port_test', 0)) {
...
function spp_mangos_execute_or_sql_fallback($realmId, $soapCommand, callable $sqlWriter, array $options = array())
{
    $soapCommand = trim((string)$soapCommand);
    $soapError = '';
    $soapResult = false;

    if ($soapCommand !== '') {
        $soapResult = spp_mangos_soap_execute_command((int)$realmId, $soapCommand, $soapError);
        if ($soapResult !== false) {
            return array(
                'ok' => true,
                'mode' => 'soap',
                'soap_result' => (string)$soapResult,
                'soap_error' => '',
                'message' => (string)($options['soap_message'] ?? 'Saved via SOAP.'),
            );
        }
    } else {
        $soapError = 'SOAP command is unavailable for this write path.';
    }

    try {
        $sqlWriter();
    } catch (Throwable $e) {
        $failure = trim($soapError);
        if ($failure !== '') {
            $failure .= ' ';
        }
        $failure .= 'SQL fallback failed: ' . $e->getMessage();
        ...
        return array(
            ...
            'message' => trim($failure),
        );
    }

    $fallbackMessage = trim((string)($options['sql_fallback_message'] ?? 'SOAP was unavailable, so a direct SQL fallback write was used. This is less safe while the world server is offline.'));
    if ($soapError !== '') {
        $fallbackMessage = trim($soapError) . ' ' . $fallbackMessage;
    }
...
function subtitle($tit, $sz='90%') {
    $subheaderLeft = spp_modern_image_url('headers/subheader/subheader-left-sword.gif');
    $subheaderRight = spp_modern_image_url('headers/subheader/subheader-right.gif');
    ?>
...
function title($tit) {
    $dateheaderLeft = spp_modern_image_url('headers/dateheader/dateheader-left.gif');
    $dateheaderBg = spp_modern_image_url('headers/dateheader/dateheader-bg.gif');
    $dateheaderRight = spp_modern_image_url('headers/dateheader/dateheader-right.gif');
    ?>
...
function header_image($img) {
    $headerBgRelative = 'headers/' . $img . 'bg.jpg';
    $headerHeadRelative = 'headers/' . $img . 'head.jpg';

    $headerBg = spp_modern_image_url($headerBgRelative);
    if (function_exists('spp_modern_image_path') && !is_file(spp_modern_image_path($headerBgRelative))) {
        $headerBg = spp_modern_image_url('headers/bannerbg.jpg');
    }

    $headerHead = spp_modern_image_url($headerHeadRelative);
    if (function_exists('spp_modern_image_path') && !is_file(spp_modern_image_path($headerHeadRelative))) {
        $headerHead = spp_modern_forum_image_url('forum_top.png');
    }
    ?>
...
function header_image_gif($img) {
    $headerBg = spp_modern_image_url('headers/' . $img . '_bg.jpg');
    $headerLeft = spp_modern_image_url('headers/' . $img . '_left.gif');
    $headerImage = spp_modern_image_url('headers/' . $img . '_image.gif');
    $headerRight = spp_modern_image_url('headers/' . $img . '_right.gif');
?>
    <table 
	cellspacing="0" 
	cellpadding="0" 
